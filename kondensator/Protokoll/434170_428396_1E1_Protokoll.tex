...
Im folgenden Graphen sind die Spannungen und Stromwerte, mit linarer Regression geplottet.

 \begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{plots/lineare_regression.pdf}
  \caption{Schaltplan Oszilloskop}
  \centering
  \end{figure}


Im Residualplot ist zu sehen, das unsere Messwerte gleichmäßig um die linare Regression streuen, wobei diese immer außer zweimal in der Statistischen Ungenauigkeit liegt.
Es liegt also keine Systematik vor, weshalb wir mit den Ergebnissen aus der linaren Regression fortfahren um den Wiederstand und seine Unsicherheit zu bestimmen.
...
\begin{equation}
    \sigma_{U\_Resultierend} = \sqrt{(R \sigma_I)^2 + (\sigma_U)^2}
\end{equation}
