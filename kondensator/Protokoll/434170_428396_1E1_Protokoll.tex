...
\begin{aufgabe}{Vorversuch: Charakterisierung der verwendeten Bauteile}
  ...
  Bevor wir mit dem Tatsächlichen Versuch angefangen haben, haben wir mit dem Multimeter zunächst die Bauteile Charakterisiert. Also Wiederstand und Kondensator gemessen. 
  ...
  Hierfür haben wir den Wiederstand auf die Steckplatte gesteckt und parallel dazu den Wiederstand gemessen.  Den Selben aufbau haben wir für den Kondensator verwendet. Der Skizze unten können Sie den Schaltplan für die Messungen entnehmen.
  
  \begin{figure}[H]
  \centering
  \subfloat[Schaltplan messen von R]{\includegraphics[width=0.5\textwidth]{Schaltung_R_Multimeter.pdf}\label{fig:f1}}
  \hfill
  \subfloat[Schaltplan messen von C]{\includegraphics[width=0.5\textwidth]{Schaltung_R_Multimeter.pdf}\label{fig:f2}}
  \caption{Schaltplan für Messungen mit dem Multimeter}
\end{figure}

Dabei haben wir zur Messung des Wiederstands eines der Kabel an die Masse des Multimeters angeschlossen, und das andere Kabel an die Spannungsbuxe des Multimeters. Da wir wussten, dass wir ein Wiederstand im Bereich 1k$\Omega$ erwarten. Haben wir Beim dreregler auf 2 k$\Omega$ gestellt.\\ 

Das selbe Vorgehen, haben wir beim Messen des Kondensators gewählt, Wobei wir hier das zweite Kabel in die buxe für mA gesteckt, und den Drehregler auf 20 $\mu$F geregelt. \\

Aus dieser Messung haben wir die Folgenden Werte erhalten. Wobei wir die Daten für die Unsicherheit der Bedienungsanleitung entnehmen konnten.  
...
\begin{table}[H]
        ...
        \begin{tabularx}{0.85\textwidth}{X X X X}
            \toprule
            \textbf{Messung} & \textbf{Zeit} & \textbf{Spannung} & $\approx$ $\tau$ \\
            \midrule
            1. Endladen     & $t_1 = 600 \mu s$ & $U_1 = 6.64V$ & $\tau \approx 10.3ms$ \\
            am Wiederstand  & $t_2 = 40.4 ms $  & $U_2 = 160mV$ \\
            \midrule
            2. Endladen     & $t_1 = 600 \mu s$ & $U_1 = 6.44V$ \\
            am Wiederstand  & $t_2 = 40.4 ms $  & $U_2 = 160mV$ \\
            \bottomrule
            1. Endladen     & $t_1 = 400 \mu s$ & $U_1 = 6.76V$ & $\tau \approx 10.1ms$ \\
            am Kondensator  & $t_2 = 31.2 ms $  & $U_2 = 320mV$ \\
            \midrule
            2. Endladen     & $t_1 = 400 \mu s$ & $U_1 = 6.76V$ \\
            am Kondensator  & $t_2 = 31.2 ms $  & $U_2 = 320mV$ \\
            \bottomrule
            1. Aufladen     & $t_1 = 4.5 ms  $ & $U_1 = -4.08V$ & $\tau \approx 9.83ms$ \\
            am Wiederstand  & $t_2 = 22.6 ms $  & $U_2 = -720mV$ \\
            \midrule
            2. Aufladen     & $t_1 = 4.5 ms  $ & $U_1 = -4.08V$ \\
            am Wiederstand  & $t_2 = 22.6 ms $  & $U_2 = -680mV$ \\
            \bottomrule
            1. Aufladen     & $t_1 = 3.4 ms  $ & $U_1 = -2.16V$ & $\tau \approx 10.00ms$ \\
            am Kondensator  & $t_2 = 18.0 ms $  & $U_2 = -6.04V$ & $ U_0 = -7.22V$ \\
            \midrule
            2. Aufladen     & $t_1 = 3.4 ms  $ & $U_1 = -2.16V$ \\
            am Kondensator  & $t_2 = 18.0 ms $  & $U_2 = -6.04V$ \\
            \bottomrule
        \end{tabularx}
        \caption{Übersicht der Einstellungen am Oszilloskop}
        \label{tab:mytable}
