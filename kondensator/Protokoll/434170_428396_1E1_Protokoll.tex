...
Hier ist ein typischer Entladeverlauf vom Kondensator zu sehen, welche wir mit dem Progrmam Cassy\_Wiederstand generiert haben.
...
Zum bestimmen der Offsets am den Kondensator haben wir den Mittelwert der Datenpunkte genommen, welche wir vor dem aufladen haben.
Da wir einen Pretrigger von 500 Messpunkten eingestellt hatten, haben wir dort genügent Datenpunkte um den Mittelwert zu bestimmen.
Dabei haben wir allerdings die letzten 10 Messpunkte nicht mit einbezogen, da diese durch den Trigger ausgelöst wurden und dort die Messung schon gestartet ist.
Das dieses auch wirklich gewähleistet ist, erkennt an daran, das in der Graphischen darstellung nur Rauschen vorhanden ist.
\begin{figure}[H]
  \centering
  ...
  \centering
\end{figure}
Dabei ist der $I_{Offset} = 1.22 * 10^{-5} A$
Dabei ist der $U_{Offset} = 0.0098 V$

Da wir davon ausgehen, das alle Spannungswerte die angenommen werden gleich warscheinlich sind, haben wir den Mittelwert der Spannungswerte genommen.
Gleiches gilt für die bestimmung der Offsets des Stroms.
Hier einmal den Ausschnitt dargestellt, welche wir zum bestimmen der Offsets genommen haben.
...
\begin{aufgabe}{Zusammenfassung und Diskussion}
  ...
  $\SI{5}{\percent}$).  
Wie der Graph eindeutig zeigt, hat die Aufladung des Kondensators noch nicht begonnen, und damit kann man diese Werte gut für die Offsetbestimmung verwenden.
 

Den Offset auf $U_0$ haben wir beim entladen des Kondensator vorm entladen vermessen.
Auch hier wurde wieder der Mittelwert, der ersten 490 Messpunkte genommen, welche unten zu sehen ist.
\end{aufgabe}
